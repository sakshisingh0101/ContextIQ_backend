...
const registerUser = asyncHandler(async (req, res) => {
    ...
    try {
        await redisClient.set(`contextiq:otp:${normalizedEmail}`, otp, {
            EX: 300
        });
    } catch (error) {
...
const verifyEmail = asyncHandler(async (req, res) => {
  ...
  try {
    savedOtp = await redisClient.get(`contextiq:otp:${normalizedEmail}`);
  } catch (error) {
  ...
  try {
    const  redisResult=await redisClient.del(`contextiq:otp:${normalizedEmail}`);
